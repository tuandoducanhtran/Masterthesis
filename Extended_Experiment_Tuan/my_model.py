...
def My_Model(weights_path = None):
    vgg = tf.keras.applications.VGG16(input_shape=(64, 64, 3), weights=None, include_top=False)
    X = Flatten()(vgg.output)
    # pred_gender = tf.keras.layers.Dense(2)(resnet.output)
    # pred_age = tf.keras.layers.Dense(9)(resnet.output)
    # pred_race = tf.keras.layers.Dense(5)(vgg.output)
    ...
    model = tf.keras.models.Model(inputs=vgg.input, outputs=[pred_gender , pred_age , pred_race])
