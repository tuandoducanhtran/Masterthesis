"""
Created on Sat Aug  6 14:36:30 2022
## this script is used to evaluate the Fairness Definitions on the model Lightface
## we read the output file with the predictions and the actual label 
## we then add our defined Fairness measures
## the multilabel confusion matrix is split into small binary classification matrixes
## the fairness definition can now be calculated with privileged and unprivileged group
## the last step is to calculate the variance out of the fairness definitions in and save the results as .csv
...
with open (r"C:\Users\tuanm\Desktop\MasterThesis\Dataset\Evaluation\Labels\utkface_label_eval_final.csv") as label_train_csv:
        csv_reader = reader(label_train_csv)
        fairface_label_all = list(csv_reader)
        
# open output from csv as list output_deepface
with open (r"C:\Users\tuanm\Desktop\MasterThesis\Output\Extended\Resnet_utk\on_UTK\output_resnetutk_on_utk_final.csv") as output_deepface_csv:
...
y_label_age = [elements.replace("0-2", "0-2") for elements in y_label_age]
y_label_age = [elements.replace("3-9", "3-9") for elements in y_label_age]
y_label_age = [elements.replace("10-19", "10-19") for elements in y_label_age]
...
y_label_age = [elements.replace("50-59", "50-59") for elements in y_label_age]
y_label_age = [elements.replace("60-69", "60-69") for elements in y_label_age]
y_label_age = [elements.replace("more than 70", "more than 70") for elements in y_label_age]
...
y_label_race = [elements.replace("Latino Hispanic", "Others") for elements in y_label_race]
...
# f = open (r"C:\Users\tuanm\Desktop\MasterThesis\Output\Extended\vgg16_utk\on_utk\fairness_vgg16_utk_on_utk_final.csv", "w", newline ="")
# writer = csv.writer(f)
